...
st.set_page_config(page_title="HYDAC Best Agent V6", layout="wide")
...
def recover_root_website_from_customer_urls(block_text: str, selected_email: str = "", company: str = "") -> str:
    """
    Some customer signatures use only image URLs such as:
    https://www.fleetpride.com/imagesns/cms/logo@200.png

    The full URL is not a website and must not be exported, but the root domain
    can be safely recovered only when it belongs to the selected customer.
    """
    selected_domain = email_domain(selected_email)
    selected_root = selected_domain.split(".", 1)[0] if selected_domain else ""
    company_token = re.sub(r"[^a-z0-9]", "", (company or "").lower())

    for raw in URL_RE.findall(block_text or ""):
        decoded = decode_redirect_url(raw)
        clean = decoded.strip("<>.,);] ")
        if not clean:
            continue
        ...
        if not host or "." not in host:
            continue
        if any(bad in host for bad in BAD_WEBSITE_PATTERNS):
            continue
        if any(domain in host for domain in INTERNAL_DOMAINS):
            continue
        if host in FREE_EMAIL_DOMAINS:
        ...
        host_token = re.sub(r"[^a-z0-9]", "", host_root)

        owned_by_customer = False
        if selected_domain and (host == selected_domain or host.endswith("." + selected_domain) or selected_domain.endswith(host)):
            owned_by_customer = True
        if selected_root and host_root == selected_root:
            owned_by_customer = True
        if company_token and host_token and (host_token in company_token or company_token in host_token):
            owned_by_customer = True

        if owned_by_customer:
            return "www." + host
    return ""
...
def extract_address_parts(block_text: str) -> Dict[str, str]:
    ...
    def parse_city_state_zip(city_line: str):
        ...
        # European/CZ style: 602 00 Brno
        m = re.search(r"^(\d{3}\s?\d{2}|\d{4,6})\s+(.+)$", city_line)
        if m:
            return m.group(2).strip(" ,"), "", m.group(1).replace(" ", "")
        return "", "", ""

    street_re = re.compile(
        r"^\s*\d{1,6}\s+[A-Za-z0-9 .'-]+\s(?:street|st\.?|road|rd\.?|ave\.?|avenue|drive|dr\.?|lane|ln\.?|blvd\.?|way|parkway|pkwy|platz|strasse|straße|square|sq\.?|court|ct\.?)\b.*$",
        re.I,
    )

    for i, line in enumerate(lines[:120]):
        if is_noise_line(line) or is_internal_text(line) or EMAIL_RE.search(line) or URL_RE.search(line):
            continue
        if street_re.search(line):
            result["Address"] = line.strip(" ,;")
            for j in range(i + 1, min(i + 5, len(lines))):
                if is_noise_line(lines[j]) or EMAIL_RE.search(lines[j]) or URL_RE.search(lines[j]):
                    continue
...
def build_reasoning(parsed, uploaded_name: str) -> Dict:
    ...
    if not website:
        website = sanitize_website(recover_root_website_from_customer_urls(selected_text, selected_email, company), selected_email)
    ...
    evidence = {
        ...
        "AttachmentDecisions": [
            ...
            for d in parsed["attachment_decisions"] if d.decision == "Keep"
        ],
        "RankedBlocks": [
            {
                "block": b.index + 1,
...
def run_ai_cleaner(parsed, uploaded_name: str, deterministic_data: Dict) -> Dict:
    ...
    prompt = f"""
You are HYDAC Best Agent V6. You are a reviewer, not a blind cleaner. Challenge deterministic fields when they are tracking links, signature images, warning banners, or product numbers misread as phones.
...
- Do not return or mention rejected attachments/images/PDFs. Only kept customer attachments matter.
...
def render_evidence(agent_data: Dict):
    ...
    st.write("**Kept customer attachments only:**")
    kept_attachment_rows = evidence.get("AttachmentDecisions", [])
    if kept_attachment_rows:
        st.dataframe(kept_attachment_rows, use_container_width=True)
    else:
        st.write("None")
