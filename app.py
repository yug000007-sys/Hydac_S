...
PHONE_LABEL_RE = re.compile(
    r"(?i)(?:tel|telephone|phone|mobile|cell|direct|office|fax|m|t|p)[:.\s-]*"
    r"(\+?\d[\d\s().\-/]{6,}\d)"
)
...
def extract_company_from_block(block_text: str, selected_email: str = "") -> str:
    ...
    # Strong exact learned examples and suffix-based company lines.
    for line in lines[:80]:
        low = line.lower().strip()
        if is_internal_text(line):
            continue
        if EMAIL_RE.search(line) or PHONE_GENERIC_RE.search(line):
            continue
        if len(line) > 90:
            continue
    ...
    # Fallback: if domain appears as readable company in nearby text, do not guess from free Gmail domains.
    if domain and domain not in {"gmail", "yahoo", "hotmail", "outlook", "aol", "icloud"}:
        for line in lines[:80]:
            if domain.lower() in line.lower() and not is_internal_text(line):
                return line.strip(" ,;-")
    return ""
...
def extract_website_from_block(block_text: str) -> str:
    for url in URL_RE.findall(block_text or ""):
        clean = url.strip(".,);]")
        low = clean.lower()
...
def extract_address_parts(block_text: str) -> Dict[str, str]:
    ...
    for i, line in enumerate(lines[:100]):
        if is_internal_text(line) or EMAIL_RE.search(line):
            continue
        if re.search(r"\b\d{1,6}\s+[A-Za-z0-9 .'-]+\b(?:street|st\.?|road|rd\.?|ave|avenue|drive|dr\.?|lane|ln\.?|blvd|way|parkway|pkwy|platz|strasse|straße)\b", line, re.I):
            result["Address"] = line.strip(" ,;")
            if i + 1 < len(lines):
                city_line = lines[i + 1].strip(" ,;")
                m = re.search(r"^(.+?),?\s+([A-Z]{2})\s+(\d{5}(?:-\d{4})?)$", city_line)
                if m:
                    result["City"], result["State"], result["ZipCode"] = m.group(1), m.group(2), m.group(3)
            break
    return result
    ...
    for raw in text.splitlines():
        ...
        if any(re.search(p, line, re.I) for p in REQUEST_STOP_PATTERNS):
            if started:
                break
            continue
        if EMAIL_RE.search(line) and len(line) < 80:
            continue
        if PHONE_GENERIC_RE.search(line) and len(line) < 80:
            continue
        if re.search(r"(?i)quote|quotation|rfq|request|need|looking for|please|can you|could you|price|availability|filter|pump|part|model|serial|drawing|attached", line):
            started = True
            lines.append(line)
        elif started:
            if len(lines) < 6 and not is_internal_text(line):
                lines.append(line)
            else:
                break
    if not lines:
...
def extract_product(text: str, attachment_names: List[str]) -> str:
    ...
    for pattern in patterns:
        ...
        if m:
            return clean_text(m.group(0 if m.lastindex is None else 1)).strip(" .,:;")
...
def build_reasoning(parsed, uploaded_name: str) -> Dict:
    ...
    website = extract_website_from_block(selected_text)
...
def run_ai_cleaner(parsed, uploaded_name: str, deterministic_data: Dict) -> Dict:
    ...
    prompt = f"""
You are HYDAC Best Agent V2. You are NOT allowed to choose a different customer unless deterministic evidence is clearly impossible.
