...
with st.sidebar:
    st.header("Settings")
    provider = st.radio("AI Provider", ["claude", "openai"],
                        format_func=lambda x: "Claude (Sonnet 4.6)" if x == "claude" else "OpenAI (GPT-4o)")
    ...
    if api_key:
        ...
        st.warning("Enter an API key to run the agent")
