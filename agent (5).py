...
def _search_ddg(query: str, max_results: int) -> List[Dict]:
    """Try DuckDuckGo via the duckduckgo_search package."""
...
def _search_bing(query: str, max_results: int) -> List[Dict]:
    """Scrape Bing search result snippets — works without an API key."""
    ...
    for li in soup.select("li.b_algo")[:max_results]:
        ...
        body_el  = li.select_one(".b_caption p, .b_algoSlug")
...
def _search_ddg_html(query: str, max_results: int) -> List[Dict]:
    """Scrape DuckDuckGo HTML search (no JS, very reliable)."""
    import urllib.request, urllib.parse
    headers = {
    ...
    results = []
    soup = BeautifulSoup(html, "html.parser")
    for div in soup.select(".result__body, .result")[:max_results]:
        title_el = div.select_one(".result__a, .result__title a")
...
def web_search(query: str, max_results: int = 5) -> List[Dict]:
    """Search with multiple fallbacks: DDG package → DDG HTML → Bing HTML."""
    errors = []

    # 1. Try duckduckgo_search package
    try:
        results = _search_ddg(query, max_results)
        if results:
            return results
    except Exception as e:
        errors.append(f"DDG package: {e}")

    # 2. Try DuckDuckGo HTML scrape
    try:
    ...
    return [{"title": "Search unavailable", "url": "", "body": " | ".join(errors)}]
